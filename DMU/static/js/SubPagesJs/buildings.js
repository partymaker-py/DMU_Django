const multiItemSlider = (function () {
    return function (selector, config) {
        ...
        const wrapperWidth = parseFloat(getComputedStyle(sliderWrapper).width);// getComputedStyle() возвращает объект, содержащий значения всех CSS-свойств элемента, полученных после применения всех активных таблиц стилей, и завершения базовых вычислений значений, которые они могут содержать. Некоторые CSS-свойства доступны через API, предоставляемые объектом, или индексацию по именам CSS-свойств.
        const itemWidth = parseFloat(getComputedStyle(sliderItems[0]).width);// parseFloat() - приводит стрковое значение к десятичной дроби
